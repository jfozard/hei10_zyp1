...
def generate_figure(input_comp_prefixes, output_fn, hists=(1,2)):
...
#                panels.append(sg.TextElement(XS, 7, data[2], size=12, anchor="middle"))
    ...
    top_labels = sg.GroupElement([ sg.TextElement(idx*XS + 0.45*XS, 10, l, size=20, anchor='middle') for (idx, l) in enumerate(['WT', 'WT+nuc', 'OX', 'OX+nuc' ]) ])
...
prefixes = [('survey_wt_0.0_1', 'WT',(1,2)), ('interpolate_0.1', 'WT+nuc',(1,2)), ('survey_wt_ox_0.0_1.0', 'OX', (2,3)), ('interpolate_0.1_ox', 'OX+nuc', (2,3))]
generate_figure([(sim_input_path + p[0], p[1], p[2]) for p in prefixes], output_path+'fig4_s2.svg')
