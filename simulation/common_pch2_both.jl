...
function simulate_multi(idx, args; tc=false)
    ...
    Q = rand(mt, Poisson(args["mean_parts"]))

    g = Gamma(args["length_gamma_alpha"], args["length_gamma_theta"])

    for q = 1:Q

        # Generate SC length, sampled from Gamma distribution)
        L::Float64 = rand(mt, g)
        push!(L_array, L)
...
function simulate_all_tc(args, io)
    ...
    Threads.@threads for i in start:start+(n-1)
        t, u_t_array, c_t_array, p_t_array, x_array, L_array = simulate_multi(i, n_args; tc=true)
...
# Parse command line options and parameters
	...
	"--C0"
        ...
        "--mean_parts"
            arg_type=Float64
            default=13.92          # Number of chromosomes
